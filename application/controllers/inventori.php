...
class Inventori extends CI_Controller
{
	...
	public function index()
	{
		...
		else
		{
			...
			$this->load->view('lobc/dashboard',$data);
			$this->load->view('lobc/awal');
		}
	}
	...
	public function surat($id){
        ...
        $this->fpdf->cell(45 ,6, ': Rp 2.500.000 / pax' ,0,0,'L',0);    
        $this->fpdf->Ln(4);
        ...
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '3. Change Schedule' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': USD 25 / Pax' ,0,0,'L',0); 
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '4. Change Name & Core name (sebelum issue ticket)' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': 10% dari jumlah group' ,0,0,'L',0); 
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '5. Materializes' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': Tidak ada materializes' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '6. Ticket group Non-Refundable and non charge name' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ' ' ,0,0,'L',0); 
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '7. Full payment time limit' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': 30 Hari sebelum keberangkatan' ,0,0,'L',0); 
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '8. Insert name time limit' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': 40 Hari sebelum keberangkatan' ,0,0,'L',0);  
        $this->fpdf->Ln(4);
        $this->fpdf->cell(75 ,6, '9. Issued ticket time limit' ,0,0,'L',0); 
        $this->fpdf->cell(45 ,6, ': 30 Hari sebelum keberangkatan' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        if ( $visa == "Y") {
           $visa = "Include Visa";
           ...
           $visa = "Exclude Visa";
        $this->fpdf->cell(75 ,6, '10. Visa' ,0,0,'L',0); 
        ...
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '2. Pembayaran dilakukan dengan Rupiah (IDR) berdasarkan regulasi Pemerintah Indonesia.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '3. Besarnya nilai tukar uang (IDR) berlaku pada saat transaksi ticket (Deposit dan Pelunasan).' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '4. Deposit idak dapat dikembalikan atau diubah ke PNR yang lain.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '5. Bilamana ada pembatalan dan tidak ada pelunasan dalam pemesanan 3 minggu sebelum keberangkatan maka deposit dianggap hilang atau hangus.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '6. Pembayaran diterima pada hari senin dampai hari jumat berupa pembayaran transfer dan tunai. Hari sabtu hanya menerima pembayaran tunai.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '7. Pembukuan yang tidak sesuai degan ketentuan diatas : AUTO CANCEL tanpa pemberitahuan sebelumnya.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '8. Pembayaran dapat diterima secara tunai ata transfer bank ke rekening kami.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '    Rekening Mandiri IDR 1210020161117 PT Dream Tours & Travel.' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '9 Untuk pembayaran transfer, wajib memberikan information atau berita pada slip transfer : Nama Perusahaan, Jumlah Pax, dan Tanggal Keberangkatan. ' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '   Serta wajib mengkonfirmasi ke kasir (Telp: 021 21381090).' ,0,0,'L',0);
        $this->fpdf->Ln(4);
        $this->fpdf->cell(185 ,6, '10. Jika tidak ada konfirmasi dan informasi transfer, pembayaran tidak akan dianggap sebagi pebayaran.' ,0,0,'L',0);
        ...
        //Line break
        $this->fpdf->Output('LOBC  '.$no.'.pdf','D');
        $pdf->Output();
	}
