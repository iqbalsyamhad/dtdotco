...
class Vtweb extends CI_Controller {
	...
	public function vtweb_pay($idpayment)
	{
		$baris = $this->db->query('select * from ol_invoice_payment a join ol_invoice b on a.idinvoice = b.idinvoice where idpayment = "'.$idpayment.'"');

		foreach ($baris->result() as $r) {
			$nmpaketumroh = $this->model_crud->GetValue('ol_paketumroh','nmpaketumroh','idpaketumroh',$r->idpaketumroh);

			$transaction_details = array(
				'order_id' 		=> $idpayment,
				'gross_amount' 	=> $r->paybill
			);
			...
			$items = [
				array(
					...
					'price' 	=> $r->paybill,
					'quantity' 	=> 1,
					'name' 		=> 'Pembayaran Termin ke-'.substr($idpayment, 14, 2)
				)
			];
	...
	public function vtweb_checkout()
	{
		...
		$items = [
			array(
				...
				'price' 		=> 45000000,
				'quantity' 	=> 1,
				'name' 			=> 'Umroh Exclusive 9 Hari'
			)
		];

		// Populate customer's billing address
		$billing_address = array(
			'first_name' 		=> "Iqbaluddin",
			'last_name' 		=> "Syam Had",
			'address' 			=> "Karang Gondang",
			'city' 					=> "Banjarnegara",
			'postal_code' 	=> "53473",
			'phone' 				=> "082136106764",
			'country_code'	=> 'IDN'
